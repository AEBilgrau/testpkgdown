...
#' @examples
#' set.seed(1)
#' sim <- testpkgdown:::SimulateGMMData(n = 100)
#' x <- sim$z
#' true.theta <- sim$theta
#' init.theta <- testpkgdown:::rtheta()  # Generate starting parameters
#'
#' # Do one EM interation
#' es <- testpkgdown:::EStep(x, init.theta)
#' new.theta <- testpkgdown:::MStep(x, es)
#'

  ...
  for (d in 3:4) {
    for (m in 3) {
      ...
      for (trace_theta in c(TRUE, FALSE)) {
        suppressWarnings({
          res <-  testpkgdown:::EMAlgorithm(dat$z,
                                     m = m,
                                     max.ite = 100,
...
test_that("EMAlgorithm warns if max iterations is hit", {
  expect_warning(
    testpkgdown:::EMAlgorithm(dat$z, theta = start.theta, max.ite = 2)
  )
})

test_that("EMAlgorithm is verbose if asked", {
  expect_output(
    testpkgdown:::EMAlgorithm(dat$z, theta = start.theta, verbose = TRUE)
  )
})

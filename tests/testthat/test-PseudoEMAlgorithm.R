...
suppressWarnings({
  res <- testpkgdown:::PseudoEMAlgorithm(uhat, meta2full(init.par, d = d),
...
test_that("PseudoEMAlgorithm expected errors & warnigns", {
  expect_error(testpkgdown:::PseudoEMAlgorithm(uhat, meta2full(c(0.5, 25, 0.1, 0.8), d = d)))
  expect_warning(testpkgdown:::PseudoEMAlgorithm(uhat, meta2full(c(0.5, 2,  0.1, 0.8), d = d),
                                          max.ite = 2))
})

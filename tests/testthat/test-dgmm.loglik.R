...
ans1 <- testpkgdown:::dgmm.loglik(theta = theta, z = z)
ans2 <- testpkgdown:::dgmm.loglik(theta = theta, z = z,
...
# Test wrong inputs
theta$d <- 1e2
expect_that(suppressWarnings(testpkgdown:::dgmm.loglik(theta = theta, z = z)),
            throws_error("formatted"))

theta$d <- d
expect_that(testpkgdown:::dgmm.loglik(theta = theta, z = z[,1]),
            throws_error("not a matrix"))
expect_that(testpkgdown:::dgmm.loglik(theta = theta, z = z[,1:2]),

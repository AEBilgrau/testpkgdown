...
ans1 <- testpkgdown:::dgmm.loglik.marginal(theta = theta, x = z)
ans2 <- testpkgdown:::dgmm.loglik.marginal(theta = theta, x = z,
...
# Test wrong inputs
theta$d <- 1e2
expect_that(suppressWarnings(testpkgdown:::dgmm.loglik.marginal(theta = theta, x = z)),
            throws_error("formatted"))

theta$d <- d
expect_that(testpkgdown:::dgmm.loglik.marginal(theta = theta, x = z[,1]),
            throws_error("not a matrix"))
expect_that(testpkgdown:::dgmm.loglik.marginal(theta = theta, x = z[,1:2]),

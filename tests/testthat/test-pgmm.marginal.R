...
for (d in seq_len(3) + 1) {
  for (n in trunc(seq(50, 5000, length.out = 20))) {
    ...
    test_that("pgmm.marginal returns proper format", {
      ...
      ans <-
        testpkgdown:::pgmm.marginal(z = sim$z,
    ...
    test_that("pgmm.marginal fails when intended", {
      ...
      sim <- SimulateGMMData(n = n,
                             theta = rtheta(d = d, method = "EqualEllipsoidal"))
      suppressWarnings({
        expect_error(testpkgdown:::pgmm.marginal(z = sim$z, theta = c(0.5, 1, 1, 0.5)),
                     "theta is formatted incorrectly")
        expect_error(testpkgdown:::pgmm.marginal(z = sim$z[1, ], theta = rtheta(d = d)),
                     "not a matrix")
        expect_error(testpkgdown:::pgmm.marginal(z = sim$z, theta = rtheta(d = 1)),
                     "Number of columns")
      })
    })
